          ...
          canvas.toBlob(
            (blob) => {
              ...
              resolve({
                ...
                compressionRatio: ((1 - blob.size / imageFile.size) * 100).toFixed(1),
              });
            },
            'image/webp',
            quality
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        ...
        <img src={originalSrc} alt="Original" className="w-full h-auto rounded-md border border-gray-700" />
      </div>
      {compressedSrc && (
        <div>
          <p className="text-xs text-gray-400 mb-2">压缩后</p>
          <img src={compressedSrc} alt="Compressed" className="w-full h-auto rounded-md border border-green-600/50" />
        </div>
      )}
    </div>
  );
