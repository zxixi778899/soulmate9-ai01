...
export default function SupportAgent() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      // Non-streaming JSON payload (fallback/error) — render it directly so the
      // bubble never stays empty.
      ...
      if (contentType.includes('application/json')) {
        ...
        const text =
          data?.content ||
          data?.error ||
          (isZh ? '抱歉，我暂时无法回答。请稍后再试。' : 'Sorry, I could not answer that. Please try again.');
        setMessages(prev => {
          const updated = [...prev];
    ...
    <>
      ...
      {!open && (
        ...
        >
          ...
          <span className="hidden md:block absolute inset-0 rounded-full bg-fuchsia-500 opacity-40 animate-ping" />
          {/* Button */}
          <span className="relative flex items-center justify-center w-11 h-11 md:w-14 md:h-14 rounded-full bg-gradient-to-br from-fuchsia-500 to-violet-600 shadow-lg shadow-fuchsia-500/30 transition-transform group-hover:scale-110">
