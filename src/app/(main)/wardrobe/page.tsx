...
function girlImage(girl?: Girlfriend): string {
  // 与 chats 页同一优先级：专用头像 > 立绘 > 生成图，避免场景图当头像
  return girl?.avatar_url || girl?.portrait_url || girl?.image_url || '';
}
...
export default function WardrobePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!girlsResponse.ok) throw new Error(girlsData.error || '伴侣加载失败');
    ...
    <main className="min-h-screen bg-[#08080d] p-4 text-white md:p-8">
      <div className="mx-auto max-w-none space-y-6">
        ...
        {loading ? <div className="flex justify-center py-32"><Loader2 className="h-8 w-8 animate-spin text-pink-400" /></div> : <div className="grid gap-6 lg:grid-cols-[280px_1fr_360px]">
          {/* 伴侣列表头像按需压缩（96px 档），压缩失败自动回退原图 */}
          <section className="space-y-3"><h2 className="text-xs font-semibold uppercase tracking-[.2em] text-zinc-500">1. 选择伴侣</h2>{girls.map((girl) => <button type="button" key={girl.id} onClick={() => setGirlId(girl.id)} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left ${girlId === girl.id ? 'border-pink-400 bg-pink-400/10' : 'border-white/10 bg-white/[.03]'}`}>{girlImage(girl) ? <OptimizedImg src={girlImage(girl)} size="thumb" previewWidth={96} alt={girl.name} className="h-12 w-12 rounded-xl object-cover object-top" /> : <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-zinc-900"><UserRound className="h-5 w-5" /></div>}<span className="flex-1 font-medium">{girl.name}</span>{girlId === girl.id && <Check className="h-4 w-4 text-pink-400" />}</button>)}{girls.length === 0 && <Empty text="还没有个人伴侣" />}</section>

          {/* 服装网格按需压缩（512px 档） */}
          <section><h2 className="mb-3 text-xs font-semibold uppercase tracking-[.2em] text-zinc-500">2. 选择服装</h2>{outfits.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{outfits.map((item) => { const selected = item.asset_id === outfitId; const product = item.product; const video = item.asset_payload?.video_url || product?.virtual_meta?.video_url; return <button type="button" key={item.id} onClick={() => setOutfitId(item.asset_id)} className={`group overflow-hidden rounded-2xl border text-left ${selected ? 'border-fuchsia-400 ring-2 ring-fuchsia-400/20' : 'border-white/10 bg-white/[.03]'}`}><div className="relative aspect-[2/3] bg-zinc-900">{product?.preview_url ? <OptimizedImg src={product.preview_url} size="card" alt={product.name} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><Shirt className="h-10 w-10 text-zinc-700" /></div>}{video && <Badge className="absolute right-2 top-2 bg-black/70"><Film className="mr-1 h-3 w-3" />视频</Badge>}{selected && <span className="absolute inset-0 flex items-center justify-center bg-fuchsia-500/15"><Check className="h-10 w-10 rounded-full bg-fuchsia-500 p-2" /></span>}</div><div className="p-3"><p className="font-medium">{product?.name || '服装'}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-zinc-500">{product?.description}</p></div></button>; })}</div> : <Empty text="衣柜还是空的，请先去商城购买或赠送服装" />}</section>

          {/* 换装预览立绘按需压缩（832px 档） */}
          <aside className="h-fit rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top,rgba(236,72,153,.18),transparent_45%),#101018] p-4 lg:sticky lg:top-6"><h2 className="mb-4 flex items-center gap-2 font-medium"><Sparkles className="h-4 w-4 text-pink-400" />换装预览</h2><div className="overflow-hidden rounded-2xl bg-black/40">{girlImage(selectedGirl) ? <OptimizedImg src={girlImage(selectedGirl)} size="detail" alt={selectedGirl?.name || '伴侣'} className="aspect-[2/3] w-full object-cover" /> : <div className="flex aspect-[2/3] items-center justify-center"><UserRound className="h-12 w-12 text-zinc-700" /></div>}</div><div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 text-sm"><p className="text-zinc-500">即将为</p><p className="mt-1 font-medium">{selectedGirl?.name || '未选择'} · {selectedOutfit?.product?.name || '未选择服装'}</p></div><Button className="mt-4 w-full bg-gradient-to-r from-pink-500 to-fuchsia-600" size="lg" onClick={() => void equip()} disabled={equipping || !selectedGirl || !selectedOutfit}>{equipping ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{equipping ? '正在保持人物一致性并换装…' : '换装并生成新立绘'}</Button><p className="mt-3 text-center text-xs leading-5 text-zinc-500">保留伴侣脸型、发色、眼睛和身材特征。商品附带视频时会同步到她的个人视频。</p></aside>
        </div>}
      </div>
    </main>
  );
