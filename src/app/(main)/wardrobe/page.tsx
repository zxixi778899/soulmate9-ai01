    ...
    <main className="min-h-screen bg-[#08080d] p-4 text-white md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {loading ? <div className="flex justify-center py-32"><Loader2 className="h-8 w-8 animate-spin text-pink-400" /></div> : <div className="grid gap-6 lg:grid-cols-[280px_1fr_360px]">
          <section className="space-y-3"><h2 className="text-xs font-semibold uppercase tracking-[.2em] text-zinc-500">1. 选择伴侣</h2>{girls.map((girl) => <button type="button" key={girl.id} onClick={() => setGirlId(girl.id)} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left ${girlId === girl.id ? 'border-pink-400 bg-pink-400/10' : 'border-white/10 bg-white/[.03]'}`}>{girlImage(girl) ? <img src={girlImage(girl)} alt={girl.name} className="h-12 w-12 rounded-xl object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-zinc-900"><UserRound className="h-5 w-5" /></div>}<span className="flex-1 font-medium">{girl.name}</span>{girlId === girl.id && <Check className="h-4 w-4 text-pink-400" />}</button>)}{girls.length === 0 && <Empty text="还没有个人伴侣" />}</section>
          ...
          <aside className="h-fit rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top,rgba(236,72,153,.18),transparent_45%),#101018] p-4 lg:sticky lg:top-6"><h2 className="mb-4 flex items-center gap-2 font-medium"><Sparkles className="h-4 w-4 text-pink-400" />换装预览</h2><div className="overflow-hidden rounded-2xl bg-black/40">{girlImage(selectedGirl) ? <img src={girlImage(selectedGirl)} alt={selectedGirl?.name || '伴侣'} className="aspect-[3/4] w-full object-cover" /> : <div className="flex aspect-[3/4] items-center justify-center"><UserRound className="h-12 w-12 text-zinc-700" /></div>}</div><div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 text-sm"><p className="text-zinc-500">即将为</p><p className="mt-1 font-medium">{selectedGirl?.name || '未选择'} · {selectedOutfit?.product?.name || '未选择服装'}</p></div><Button className="mt-4 w-full bg-gradient-to-r from-pink-500 to-fuchsia-600" size="lg" onClick={() => void equip()} disabled={equipping || !selectedGirl || !selectedOutfit}>{equipping ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{equipping ? '正在保持人物一致性并换装…' : '换装并生成新立绘'}</Button><p className="mt-3 text-center text-xs leading-5 text-zinc-500">保留伴侣脸型、发色、眼睛和身材特征。商品附带视频时会同步到她的个人视频。</p></aside>
        </div>}
      </div>
    </main>
  );
