...
const TEST_LIMIT = { maxRequests: 20, windowMs: 60 * 1000 };

/**
 * POST /api/admin/ai-modules/test
 * Ad-hoc connectivity test for a chat endpoint (no DB involved).
 *
 * Body: { provider, model_id, api_base_url?, api_key_env?, temperature?,
 *         max_tokens?, cost_per_1k_input?, cost_per_1k_output?, prompt? }
 */
export async function POST(request: NextRequest) {
  const admin = await requireAdmin(request, 'admin');
  if (admin.error) return admin.error;

  const rl = await checkRateLimitAsync(`admin-ai-test:${admin.user!.id}`, TEST_LIMIT);
  if (!rl.allowed) {
  ...
  try {
    ...
    if (!provider || !model_id) {
      return NextResponse.json(
        { error: 'provider and model_id are required' },
        { status: 400 },
      );
    }
    ...
    logger.error('admin/ai-modules/test failed', { e });
    return NextResponse.json(
