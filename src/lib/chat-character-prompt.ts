...
export function buildCharacterPrompt(input: CharacterPromptInput): string {
  ...
  if (zh) {
    sections.push(
      ...
      '5) 【语言锁·跟随界面】当前界面是中文：全程简体中文。禁止中英夹杂、禁止整段英文。用户打英文也要用中文回。',
      '6) 不要列表、不要说教、不要「作为AI」。',
      `7) 说话方式：${styleLine}`,
    ...
    sections.push(
      ...
      '5) [LANGUAGE LOCK · FOLLOWS UI] App UI is English → English-only replies. ZERO Chinese characters (汉字). Even if he types Chinese, reply in English.',
