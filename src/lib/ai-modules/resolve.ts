import type { AiModulesConfig, AppLocale, MembershipTier, ModelEndpoint, ResolveChatContext, ResolveImageContext, ResolvedChatCall, ResolvedImageCall, TierChatRoute } from './types';
...
function tierOf(tier?: MembershipTier): 'free' | 'basic' | 'pro' | 'premium' | 'unlimited' { return tier === 'admin' || tier === 'unlimited' ? 'unlimited' : tier === 'premium' ? 'premium' : tier === 'pro' ? 'pro' : tier === 'basic' ? 'basic' : 'free'; }
/** Runtime-safe route lookup: legacy DB configs may miss newer tiers (premium/basic). */
function routeFor(cfg: AiModulesConfig, tier: 'free' | 'basic' | 'pro' | 'premium' | 'unlimited'): TierChatRoute {
  return cfg.chat.tiers[tier] || cfg.chat.tiers.pro || cfg.chat.tiers.free;
}
...
export function resolveImageCall(input: AiModulesConfig | null | undefined, ctx: ResolveImageContext): ResolvedImageCall {
  ...
  const daily = item.daily_limit_override ?? (tier === 'unlimited' ? cfg.image.unlimited_daily_images : (tier === 'pro' || tier === 'premium') ? cfg.image.pro_daily_images : cfg.image.free_daily_images);
